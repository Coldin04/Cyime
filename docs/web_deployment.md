...
`PUBLIC_API_BASE_URL` 也会写入前端公开的 `/skill.md`、`/manifest.json` 和 `/openapi.json`，用于声明 MCP、REST Open API 和 Skill OAuth 地址。后端部署应同步配置 `API_BASE_URL` / `PUBLIC_API_BASE_URL` 指向同一个后端公网 origin；如果启用 Skill OAuth，还需要在后端配置 `PUBLIC_BASE_URL` 和按需配置 `CYIME_SKILL_OAUTH_REDIRECT_URIS`。登录后的授权确认页由前端在 `${PUBLIC_BASE_URL}/auth/skill/consent` 渲染。
